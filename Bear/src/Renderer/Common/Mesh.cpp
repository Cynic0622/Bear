...
namespace Bear {
	...
	{
		//m_VertexBuffer = std::make_unique<VulkanBuffer>(device, sizeof(Vertex) * vertices.size(), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
		m_VertexBuffer = device.CreateBuffer(sizeof(Vertex) * vertices.size(), BufferUsage::VertexBuffer, true);
		//m_IndexBuffer = std::make_unique<VulkanBuffer>(device, sizeof(uint16_t) * indices.size(), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
	...
	void Mesh::Bind(VulkanCommandBuffer& commandBuffer) const
	{
		VulkanBuffer* vkVertexBuffer = static_cast<VulkanBuffer*>(m_VertexBuffer.get());
		VulkanBuffer* vkIndexBuffer = static_cast<VulkanBuffer*>(m_IndexBuffer.get());
		//VkBuffer vertexBuffers[] = { vkVertexBuffer->GetHandle() };
		// std::vector<VulkanBuffer*> vertexBuffers = { static_cast<VulkanBuffer*>(m_VertexBuffer.get())};
		std::vector<VkDeviceSize> offsets = { 0 };
		// 这两个函数设计的不太好
		commandBuffer.BindVertexBuffer(*vkVertexBuffer, 0, 0);
		commandBuffer.BindIndexBuffer(*vkIndexBuffer, 0);
	}
	void Mesh::Draw(VulkanCommandBuffer& commandBuffer) const
	{
